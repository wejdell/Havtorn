...
namespace JobWorkerMgr
{
	...
	struct WorkerState
	{
		...
		AkThreadProperties workerThreadProperties;
	...
	inline void GetWorkerThreadProps(AkThreadProperties& out_threadProps, int workerIdx)
	{
		if (g_settings.arThreadWorkerProperties)
			out_threadProps =  g_settings.arThreadWorkerProperties[workerIdx];
		else
			AKPLATFORM::AkGetDefaultHighPriorityThreadProperties(out_threadProps);
	}
	...
	AK_DECLARE_THREAD_ROUTINE(JobWorkerThread)
	{
		...
		AK_THREAD_INIT_CODE(workerState.workerThreadProperties);
	...
	AKRESULT InitWorkers(const InitSettings& in_implInitSettings)
	{
		...
		{
			GetWorkerThreadProps(pNextWorkerState->workerThreadProperties, workerIdx);
			...
			char threadName[32];
			snprintf(threadName, 32, "AK::Worker-%d", workerIdx);
			AKPLATFORM::AkCreateThread(
				JobWorkerThread,
				pNextWorkerState,
				pNextWorkerState->workerThreadProperties,
